...
final class AdminRenderer
{
    ...
    private function layout(string $title, string $body, bool $nav): string
    {
        ...
        return '<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">'
            ...
            . '.row-actions button.danger, .danger { color: #9F2F2D; }'
    ...
    private function filesModal(): string
    {
        return '<div x-dialog x-model="dialogOpen" x-cloak class="dialog-shell">'
            ...
            . '</div>'
            . '<label x-show="!dialogShowFile" x-cloak>Name<input x-ref="dialogNameInput" x-model="dialogName"></label>'
    ...
    private function filesScript(): string
    {
        return '<script>'
            ...
            . 'csrf:csrf,bucket:bucket,prefix:prefix,search:"",selectedItems:[],statusMessage:summary,statusError:false,busy:false,dialogOpen:false,dialogAction:"",dialogTitle:"",dialogMessage:"",dialogPath:"",dialogName:"",dialogShowFile:false,'
            . 'init(){},'
            ...
            . 'openDialog(title,message,action,path,name,showFile){this.dialogTitle=title;this.dialogMessage=message||"";this.dialogAction=action;this.dialogPath=path||"";this.dialogName=name||"";this.dialogShowFile=showFile;this.dialogOpen=true;this.busy=false;if(!showFile){this.$nextTick(()=>{const el=this.$refs.dialogNameInput;if(el){el.focus();el.select();}});}},'
